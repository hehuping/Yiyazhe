...
class IndexController extends Controller {
    ...
    private function getApiDate($url){
    	...
    	foreach ($goodlist as $k=>$v){
    		...
    		//echo $v->deal_class.":".$v->deal_class_id."<br>";
    		if(strpos($v->deal_taobao_link, 'taobao'))$shop="淘宝";else $shop="天猫";
    		if(strpos($v->deal_title, '中老年') !== false)continue;
    		if(strpos($v->deal_title, '妈妈') !== false)continue;
    		if(strpos($v->deal_title, '爸爸') !== false)continue;
    		if(strpos($v->deal_title, '中年') !== false)continue;
    		if(strpos($v->deal_title, '儿童') !== false)continue;
    		if(strpos($v->deal_title, '女童') !== false)continue;
    		if(strpos($v->deal_title, '老人') !== false)continue;
    		if(strpos($v->deal_title, '商务') !== false)continue;
    		$data = array(
    				'title'=>(string)$v->deal_title,
    				'class_id'=>(int)$v->deal_class_id,
	...
	public function getjiuJson(){
		...
		$dataArr = $dataobj->goodslist;

		//反转
		$goodlist = array_reverse($dataArr);
		//入库
		$goods_model = M('goods');
		//遍历商品详情数组
